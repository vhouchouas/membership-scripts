...
// TODO: mutualize the code duplicated with AddAdditionalEmail (could be done with a trait perhaps)
#[AsCommand(name: 'member:additional-email:rm')]
class RmAdditionalEmail extends Command {
	public function __construct(private MemberRepository $memberRepository, private MemberAdditionalEmailRepository $additionalEmailRepository) {
		parent::__construct();
	}
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if ($member->rmAdditionalEmail($additionalEmail, $this->additionalEmailRepository)) {
			$this->memberRepository->save($member, true);
			$output->writeln('Additional email removed');
		} else {
			$output->writeln('Member already did not had this additional email');
		}
