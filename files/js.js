...
function match(member, row) {
	var scores = slackUsers.filter(user => {
			...
			score += stringSimilarity.compareTwoStrings(user.profile["email"].toLowerCase(), member.email.toLowerCase());
			count +=1;
			}

			if(user.profile.hasOwnProperty("first_name") && user.profile.first_name.length > 0 &&
					user.profile.hasOwnProperty("last_name") && user.profile.last_name.length > 0) {
			score += stringSimilarity.compareTwoStrings(user.profile["first_name"].toLowerCase(), member.firstName.toLowerCase())
			score += stringSimilarity.compareTwoStrings(user.profile["last_name"].toLowerCase(), member.lastName.toLowerCase());
			count +=2;
			} else if(user.profile.hasOwnProperty("real_name") && user.profile.real_name.length > 0) {
...
#ffffff 10px,
			...
			slack.click(function() {
					...
					match(members[helloassoID], parent);
...
$.each( members, function( key, member) {
		...
		$(helloasso_col).append(
				member.lastRegistrationDate.date + "</br>" + 
