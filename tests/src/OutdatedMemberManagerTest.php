...
final class OutdatedMemberManagerTest extends TestCase {
  public function test_deleteExpectedUsers(){
    ...
    // // Create a doctrine mock to return the list of users
    // // This mock should also be called in order to delete old data in database
    $doctrine = $this->createMock(DoctrineConnector::class);
    $doctrine->method('getOrderedListOfLastRegistrations')->willReturn(array(
          $this->buildMemberWithEmail('userOk1'),
          $this->buildMemberWithEmail('userOk2')
        )
    );
    $doctrine->expects($this->once())->method('getOrderedListOfLastRegistrations')->with($this->equalTo(new DateTime("2019-01-01T00:00:00", new DateTimeZone("Europe/Paris"))));
    $doctrine->expects($this->once())->method('deleteRegistrationsOlderThan')->with($this->equalTo(new DateTime("2018-01-01T00:00:00", new DateTimeZone("Europe/Paris"))));
    ...
    $sut->deleteOutdatedMembersIfNeeded(new DateTime("2019-01-30Z"), $doctrine);
  ...
  public function test_dontDeleteAnyOneIfWeReNotAtATimeWhenWeShouldDeleteUsers(){
    ...
    // // Create a doctrine mock
    $doctrine = $this->createMock(DoctrineConnector::class);
    $doctrine->expects($this->never())->method('getOrderedListOfLastRegistrations');
    $doctrine->expects($this->never())->method('deleteRegistrationsOlderThan');
    ...
    $sut->deleteOutdatedMembersIfNeeded(new DateTime("2019-07-30Z"), $doctrine);
  ...
  public function test_sendMailIfOldMembersRegisteredAgain(){
    ...
    $inputData = [
      $this->buildMemberWithEmail("toto@toto.com"),
      $this->buildMemberWithEmail("titi@titi.com"),
      $this->buildMemberWithEmail("tutu@tutu.com")
    ];

    // // Create test data which will be returned by the doctrine mock. Let's consider it returns only two emails
    $doctrine = $this->createMock(DoctrineConnector::class);
    $doctrine->expects($this->once())->method('findMembersInArrayWhoDoNotRegisteredAfterGivenDate')->with($this->equalTo(array("toto@toto.com", "titi@titi.com", "tutu@tutu.com")), $this->equalTo(new dateTime("2020-01-01", new DateTimeZone("Europe/Paris"))))->willReturn(array("toto@toto.com", "titi@titi.com"));
    ...
    $sut->tellAdminsAboutOldMembersWhoRegisteredAgainAfterBeingOutOfDate($inputData, $doctrine, $emailSender);

    // No need to assert because we already set up 'expects'

  }

  private function buildMemberWithEmail(string $email) : MemberDTO {
    $res = new MemberDTO();
    $res->email = $email;
    return $res;
  }
